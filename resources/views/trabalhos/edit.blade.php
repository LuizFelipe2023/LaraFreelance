...
<div class="register-container">
    <div class="card">
        ...
        <div class="card-body">
            <form id="trabalhoForm" method="POST" action="{{ route('trabalhos.update', $trabalho->id) }}">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    ...
                    <div class="col-md-12">
                        ...
                        <input id="titulo" type="text" class="form-control @error('titulo') is-invalid @enderror"
                            name="titulo" value="{{ old('titulo', $trabalho->titulo) }}" required>
                        @error('titulo')
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea id="descricao" name="descricao" rows="6"
                            class="form-control @error('descricao') is-invalid @enderror" required>{{ old('descricao', $trabalho->descricao) }}</textarea>
                        @error('descricao')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <select id="status" class="form-select @error('status') is-invalid @enderror" name="status" required>
                            <option value="aberto" {{ (old('status', $trabalho->status) == 'aberto') ? 'selected' : '' }}>Aberto</option>
                            <option value="em_negociacao" {{ (old('status', $trabalho->status) == 'em_negociacao') ? 'selected' : '' }}>Em Negociação</option>
                            <option value="fechado" {{ (old('status', $trabalho->status) == 'fechado') ? 'selected' : '' }}>Fechado</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="d-flex justify-content-end mt-4 gap-2">
                    <a href="{{ route('trabalhos.index') }}" class="btn btn-warning text-decoration-none">Cancelar</a>
                    <button type="button" class="btn btn-primary" id="btnOpenConfirmModal">Atualizar</button>
                </div>
            </form>
        </div>
    </div>
